  ...
  <div class="lottery-card">
    <div class="card-header">
      <div class="header-left">
        ...
        <div class="issue-text">{{ $t("di") }}{{ currentIssue }}{{ $t("lun") }}</div>
      </div>
      <div class="header-right">
        <div class="status-tag" :class="{ 'betting': !isClosed, 'closed': isClosed }">
          {{ isClosed ? $t('yifengpan') :  $t('xiaduzhong') }}
        </div>
        <div class="time-text" v-if="!isClosed">
          {{ $t("xiaqishijian") }}： {{ countdownText }}
        </div>
      </div>
    </div>
    ...
    <div class="grid-section">
      <div class="number-grid">
        ...
        <div 
          v-for="i in (36 - displayCodes.length)" 
          :key="`placeholder-${i}`"
          class="grid-cell placeholder-cell"
        >
        </div>
      </div>
    </div>
    ...
    <div class="action-section">
      <button class="bet-button" @click="goToLottery">{{ $t("lijitouzhu") }}</button>
    </div>
    
    <div class="footer-info">
      <span>{{ $t("dangqian") }}{{ randomOrderCount }}{{ $t("xiadan") }}</span>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    goToLottery() {
      this.$router.push({
        path: '/lottery',
        query: { tab: this.tabIndex.toString() }
      })
    }
  }
}
